...
class Conflict(db.Model):
    ...
    aircraft1_id = db.Column(db.String(10), db.ForeignKey('aircraft.id'))
    aircraft2_id = db.Column(db.String(10), db.ForeignKey('aircraft.id'))
    distance = db.Column(db.Float)
    altitude_diff = db.Column(db.Float)
    severity = db.Column(db.String(20))
    time_to_conflict = db.Column(db.Integer)
    resolved = db.Column(db.Boolean, default=False)
    resolution_action = db.Column(db.String(200))
...
class Alert(db.Model):
    ...
    severity = db.Column(db.String(20))
    aircraft_id = db.Column(db.String(10), db.ForeignKey('aircraft.id'))
...
class ActionLog(db.Model):
    ...
    controller = db.Column(db.String(50), default='SYSTEM')
    timestamp = db.Column(db.DateTime, default=datetime.now)

# ============ HELPER FUNCTIONS ============

def generate_aircraft_id(airline, number):
    return f"{airline}{number:03d}"
...
def initialize_20_aircraft():
    ...
    for i in range(1, 21):
        ...
        aircraft_id = generate_aircraft_id(airline, flight_num)
        ...
        aircraft = Aircraft(
            ...
            status=random.choices(['ACTIVE', 'ACTIVE', 'ACTIVE', 'EMERGENCY'], weights=[0.9, 0.9, 0.9, 0.1])[0],
...
def add_aircraft():
    ...
    log = ActionLog(
        action_type='ADD',
        aircraft_id=data['id'],
        details=f"Aircraft {data['id']} added to airspace"
    )
    db.session.add(log)
    db.session.commit()
...
def delete_aircraft(aircraft_id):
    ...
    log = ActionLog(
        ...
        details=f"Aircraft {aircraft_id} removed from airspace"
    )
    db.session.add(log)
    db.session.commit()
...
def get_dashboard_stats():
    ...
    active_conflicts = Conflict.query.filter_by(resolved=False).count()
...
def initialize_database():
    ...
    aircraft_list = initialize_20_aircraft()
    ...
    return jsonify({
        'message': f'Database initialized with {len(aircraft_list)} aircraft',
        'aircraft': [a.to_dict() for a in aircraft_list]
    })
    ...
    with app.app_context():
        ...
        if Aircraft.query.count() == 0:
            ...
            for aircraft in aircraft_list:
                db.session.add(aircraft)
            db.session.commit()
            print(f"✅ Added {len(aircraft_list)} aircraft to database")
    
    print("=" * 60)
    print("🚀 ATC-DSS WITH SQLITE DATABASE (20 PLANES)")
    print("=" * 60)
    print(f"📍 API: http://localhost:5000")
    print(f"💾 Database: atc_dss_20planes.db")
    print(f"✈️  Aircraft in DB: {Aircraft.query.count()}")
    print("=" * 60)
